...
const SCRIPT_API_ROOT_INTERFACES = [
  "ScriptApi",
  "ScriptRuntimeApi",
...
const buildDeclarationMap = (
  ...
  for (const rootName of SCRIPT_API_ROOT_INTERFACES) {
    const declaration =
...
const renderScriptTypes = (
  ...
  for (const rootName of SCRIPT_API_ROOT_INTERFACES) {
    renderApiInterface(state, rootName, rootName);
  }
  ...
  const rootInterfaceOrder = new Map<string, number>(
    SCRIPT_API_ROOT_INTERFACES.map((name, index) => [name, index]),
  );
  const interfaces = Array.from(state.rendered.values())
    .sort((left, right) => {
      const leftOrder = rootInterfaceOrder.get(left.name);
      const rightOrder = rootInterfaceOrder.get(right.name);
...
const validateGeneratedTypes = (content: string): void => {
  ...
  for (const specifier of [
    ...
    if (!content.includes(`declare module "${specifier}"`)) {
      fail(`Generated scripting types must declare ${specifier}`);
    }
  }
  if (content.includes('declare module "lucent"')) {
    fail("Generated scripting types must not declare the removed lucent module");
  }
  if (content.includes('declare module "@lucent/')) {
    fail("Generated scripting types must not declare removed scoped modules");
  }
