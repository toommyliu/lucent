...
| `LiveModel.update()`, `replaceFrom()`, and `snapshot()`; `LiveEntity.addAura()`, `clearAuras()`, and `removeAura()`; `LiveMonster.replaceDrops()` | Keep the allocation-free live objects. Continue marking these methods `@internal` and exclude them from generated reference pages and editor declarations. | A script-facing wrapper would allocate another object for every live model. | Do not add readonly wrapper objects or defensive copies for this purpose. |
| `require("effect")` exposes the complete Effect, Option, and Duration namespaces while the generated declaration accepts arbitrary `any` properties | Expose a typed allowlist of effect construction, composition, recovery, timeout, Option, and Duration helpers. | Scripts get accurate autocomplete without treating every upstream Effect export as a supported Lucent API. | Keep runners, fibers, services, layers, and runtime controls out of the scripting module. |
...
| `combat.killForItem(target, item, quantity?, options?)` and the temporary-item and Army variants | Use `killForItem(target, { item, quantity? }, options?)`. | Item and quantity form one farming goal, while combat options remain solely about how to fight. The target stays positional because it is the action's primary operand. | `quantity` defaults to `1`. |
| `players.getMe()` | Remove it; use `player.get()`. | It is an exact cross-namespace alias. | Do not keep a compatibility alias. |
| `combat.hunt()` returns a monster even when its cell is missing or `player.jumpToCell()` fails | Return the resolved monster only after reaching its cell; otherwise return `null`. | The monster remains useful to callers, while `null` consistently means the hunt could not produce a reachable target. | Keep `LiveMonster \| null`; a boolean would discard which monster and cell were selected. |
| `combat.kill()` may complete when a `targetPriority` monster dies | Complete only when the requested target dies. | Priority targets control attack order; they do not replace the requested completion target. | Farming continues to use the same kill behavior while checking the item between kills. |
...
| `combat.hunt(query)`, `kill(query)`, `killForItem(query, ...)`, and `killForTempItem(query, ...)` | Use `target` for each combat action. | `target` describes the role of the input. The accepted `combat.attack(target)` rename and the Army combat functions already use it. | The farming variants now use `target`; `hunt()` and `kill()` remain pending. |
