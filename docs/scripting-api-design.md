...
| `shops.loadArmorCustomize()` and `shops.loadHairShop()` | Keep them as `shops.openArmorCustomize()` and `shops.openHairShop(shopId)`. | These functions open game UI rather than loading shop data. | Keep the capabilities and change the verbs. |
...
| `shops.getInfo()` | Rename it to `shops.getCurrent()`. | It returns the currently loaded shop, not generic information. | |
...
| Before | After | Why | Decision note |
| --- | --- | --- | --- |
| `environment.getState()` and environment mutations return the state object stored in the internal `Ref` | Return detached environment snapshots. | A script can mutate Environment state without using an Environment operation. | |
...
| Five granular Environment drop-policy setters, `setRejectUnregisteredDrops()`, and `setDropPolicy(Partial<...>)` | Keep one `updateDropPolicy(patch)` function. | The granular setters only wrap the partial update. `update` also states the patch semantics. | |
| `setAutoRegisterRequirements()` and `setAutoRegisterRewards()` | Use `updateQuestAutoRegister({ requirements?, rewards? })`. | One patch operation preserves individual updates without duplicate functions. | |
...
| Every Environment function declares `unknown` errors | Make reads infallible and mutations fail with a named `EnvironmentError`. | `unknown` makes recovery awkward. `getState()` cannot fail, while `fetchBoosts()` already catches failures. | |
| `environment.fetchBoosts()` and `environment.syncToAll()` | Rename them to `getInventoryBoosts()` and `syncToAllClients()`. | The first only examines current inventory, and the second targets every running game client. | |
