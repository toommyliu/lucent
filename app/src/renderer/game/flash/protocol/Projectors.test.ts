...
describe("Flash packet projectors", () => {
  ...
  it.effect("projects boolean temp addItems into temp inventory", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness();
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* harness.publish(
            extensionJson("addItems", {
              items: {
                1178: item(1178, "Defeated Escherion", {
                  bTemp: true,
                  iQty: 1,
                  sES: "None",
                  sType: "Quest Item",
          ...
          const tempItems = yield* items.getAll("temp");
          expect(tempItems).toHaveLength(1);
          expect(tempItems[0]?.name).toBe("Defeated Escherion");
          expect(yield* items.get("inventory", 1178)).toBeNull();
        }).pipe(Effect.provide(harness.layer)),
      );
    }),
  );

  it.effect("keeps drops state on failed getDrop", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness();
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* harness.publish(
            extensionJson("dropItem", {
              items: {
                99: item(4, "Dropped Token", { iQty: 2 }),
              },
            }),
          );
