...
it.live("duplicate job with default replacement interrupts previous job", () =>
  withJobs(openGate, (jobs) =>
    Effect.gen(function* () {
      let firstRuns = 0;
      let secondRuns = 0;
...
it.live("stop interrupts a running job", () =>
  withJobs(openGate, (jobs) =>
    Effect.gen(function* () {
      let runs = 0;
...
it.live("stopAll interrupts all running jobs", () =>
  withJobs(openGate, (jobs) =>
    Effect.gen(function* () {
      let runsA = 0;
      let runsB = 0;
...
it.effect("periodic job respects loggedIn and loggedOut through JobGate", () =>
  Effect.gen(function* () {
    let ready = false;
    const gate = {
      isOpen(runWhen) {
...
it.effect(
  "periodic cycle failure is logged and swallowed while loop continues",
  () =>
    withJobs(
      openGate,
      (jobs) =>
